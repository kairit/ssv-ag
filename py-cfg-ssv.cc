...
"first rule.\n"
"\n"
"Training file contains the training data that must be consistent with the grammar,\n"
"i.e. the labels used in the training data must be some non-terminals used in the grammar file.\n"
"\n"
...
F gibbs_estimate(pycfg_type& g, const Sss& trains,
  ...
  for (unsigned i = trains_size; i < n; ++i)
      train_flag[i] = true;
  if (train_frac == 1)
    for (unsigned i = 0; i < trains_size; ++i)
      train_flag[i] = true;
  else {
    ...
    if (train_frac_randomise) {
      ...
      std::random_shuffle(train_flag.begin(), train_flag.begin() + trains_size, rng);
    }
  }
  ...
  for (unsigned i = 0; i < n; ++i) {
    ...
    F tprob = p.restricted_inside(trains[i], spans[i], i >= trains_size, g.start);
  ...
  if (trace_stream_ptr)
    *trace_stream_ptr << "# " << nwords << " tokens in " 
		      ...
		      << "#\n"
		      << "# It\tTemp\tTime\t-logP\ttables\tsame\tchanged\treject\tdefault_pya\t(parent pym pyn pya pyb)*" << std::endl;
  ...
  for (U iteration = 0; iteration < niterations; ++iteration) {
    ...
    if (debug >= 100) {
      std::cerr << "# Iteration " << iteration << ", " 
		<< g.sum_pym() << " tables, "
		<< -g.logPcorpus()/(log(2)*nwords+1e-100) << " bits/token, " 
	...
	//      << std::endl << "## tp0 = " << tp0 
      ...
      F tprob = p.restricted_inside(trains[i], spans[i], i >= trains_size, g.start); // compute inside CKY table for proposal grammar
      if (tprob <= 0) 
	      std::cerr << "## " << HERE
...
void extract_constraints(const std::string& data, S_Span& span, Ss& terminals) {
    ...
    if (data.find("(") == std::string::npos) {
        ...
        Ss terms;
        readline_symbols(ss, terms);
        terminals.insert(terminals.end(), terms.begin(), terms.end());
        if (debug > 1000)
            std::cerr << "extracts constraints terminals: " << terminals << std::endl;
    } else {
        U brackets = 0;
        U start = 0;
        std::string nonterminal;
        U spanstart = terminals.size();
        U ind0 = 0, ind1 = 0;
        for (U i = 0; i != data.size(); ++i) {
            if (data[i] == '(') {
                if (i != 0 && brackets == 0 && i - start > 1) {
                    std::stringstream ss(data.substr(start, i - start));
                    Ss terms;
                    readline_symbols(ss, terms);
                    terminals.insert(terminals.end(), terms.begin(), terms.end());
                }
                ...
                if (brackets == 1) {
                    ind0 = data.find(" ", i+1);
                    nonterminal = data.substr(i+1, ind0-i-1);
                    spanstart = terminals.size();
                    if (debug > 1000) 
                        std::cerr << " # extract_constraints: i = " << i << "  ind0 = " << ind0 << "  nonterminal = " << nonterminal << "  spanstart = " << spanstart << std::endl;
                }
            } else if (data[i] == ')') {
                --brackets;
                if (brackets == 0) {
        ...
        if (data.size() - start > 1) {
            std::stringstream ss(data.substr(start, data.size() - start));
            Ss terms;
            readline_symbols(ss, terms);
            terminals.insert(terminals.end(), terms.begin(), terms.end());
        }
...
int main(int argc, char** argv) {
  ...
  Str parses_filename = "", grammar_filename = "", trace_filename = "", labelled_filename = "";
  Strs evalcmdstrs;
  Postreamps evalcmds;
  U eval_every = 1;
  ...
  while ((chr = getopt(argc, argv, "A:CDEF:G:H:I:N:PR:ST:X:Z:a:b:d:e:f:g:h:l:m:n:r:s:t:w:x:z:y:")) 
    ...
    case 'z':
      ...
      std::cerr << usage << std::endl << exit_failure;
    }
  ...
  U trains_size = trains.size();
  if (debug >= 1000) 
    std::cerr << "# trains.size() = " << trains_size << std::endl;
  
  if (labelled_filename != "") {
    std::ifstream labelled_is(labelled_filename.c_str());
    if (!labelled_is)
      std::cerr << "# Error in " << argv[0]
  ...
  gibbs_estimate(g, trains, spans, trains_size, train_frac, train_frac_randomise, evalcmds, eval_every,
